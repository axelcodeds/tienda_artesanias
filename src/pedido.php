...
class Pedido{
    ...
    public function registrarDetalle($_params){
        $sql = "INSERT INTO `detalle_pedido`(`pedido_id`, `artesania_id`, `precio`, `cantidad`) 
        VALUES (:pedido_id,:artesania_id,:precio,:cantidad)";

        $resultado = $this->cn->prepare($sql);

        $_array = array(
            ":pedido_id" => $_params['pedido_id'],
            ":artesania_id" => $_params['artesania_id'],
    ...
    public function mostrarDetallePorIdPedido($id)
    {
        $sql = "SELECT 
                ...
                INNER JOIN artesania pe ON pe.id= dp.artesania_id	
